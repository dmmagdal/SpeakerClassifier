# get_model_vram.py
# Take a specific pre-trained speaker classsification model and the
# associated dataset and capture the max amount of VRAM used during
...
from common.helper import get_device, get_model
...
def main():
    ...
    start_vram = pynvml.nvmlDeviceGetMemoryInfo(handle)
    start_vram = start_vram.used / (1024 * 1024)
    print(f"starting VRAM usage: {start_vram:.2f} mb")
    ...
    train_vram = pynvml.nvmlDeviceGetMemoryInfo(handle)
    train_vram = train_vram.used / (1024 * 1024)
    print(f"Peak memory during training: {peak_memory:.2f} mb")
    print(f"training VRAM usage: {train_vram:.2f} mb")
    ...
    # Get peak memory usage (in mb).
    peak_memory = torch.cuda.max_memory_allocated() / (1024 * 1024)
    infer_vram = pynvml.nvmlDeviceGetMemoryInfo(handle)
    infer_vram = infer_vram.used / (1024 * 1024)
    print(f"Peak memory during inference: {peak_memory:.2f} mb")
    print(f"inference VRAM usage: {infer_vram:.2f} mb")
